...
# ══════════════════════════════════════════════════════════════════════════════
# Multi-agent insights specialists (mini step-configs, no manager)
# ══════════════════════════════════════════════════════════════════════════════


def cheap_check_quantitative_specialist(output: Any) -> CheapCheckResult:
    if not isinstance(output, dict):
        return CheapCheckResult(False, "Output must be a JSON object.")
    findings = output.get("quantitative_findings")
    if not isinstance(findings, list):
        return CheapCheckResult(False, "'quantitative_findings' must be a list.")
    for i, f in enumerate(findings):
        if not isinstance(f, dict):
            return CheapCheckResult(False, f"quantitative_findings[{i}] is not an object.")
        if not f.get("metric"):
            return CheapCheckResult(False, f"quantitative_findings[{i}] missing 'metric'.")
        if "value" not in f:
            return CheapCheckResult(False, f"quantitative_findings[{i}] missing 'value'.")
    if not isinstance(output.get("crosstabs_run", []), list):
        return CheapCheckResult(False, "'crosstabs_run' must be a list.")
    return CheapCheckResult(True)


QUANTITATIVE_SPECIALIST_STEP_CONFIG = StepConfig(
    name="insights_quantitative_specialist",
    cheap_check=cheap_check_quantitative_specialist,
    manager_prompt="",
    manager_context_builder=lambda o, i: "",
    rubric=[],
    score_threshold=0.0,
    max_retries=0,
    use_manager=False,
...
def cheap_check_qualitative_specialist(output: Any) -> CheapCheckResult:
    if not isinstance(output, dict):
        return CheapCheckResult(False, "Output must be a JSON object.")
    for key in ("qualitative_findings", "objections_and_blockers",
                "contradictions_and_blind_spots", "hypotheses_to_test"):
        v = output.get(key)
        if v is not None and not isinstance(v, list):
            return CheapCheckResult(False, f"'{key}' must be a list when present.")
    populated = any(
        isinstance(output.get(k), list) and len(output.get(k)) > 0
        for k in ("qualitative_findings", "objections_and_blockers",
                  "contradictions_and_blind_spots", "hypotheses_to_test")
    )
    if not populated:
        return CheapCheckResult(
            False,
            "Qualitative specialist returned no themes, objections, contradictions, or hypotheses.",
        )
    return CheapCheckResult(True)


QUALITATIVE_SPECIALIST_STEP_CONFIG = StepConfig(
    name="insights_qualitative_specialist",
    cheap_check=cheap_check_qualitative_specialist,
    manager_prompt="",
    manager_context_builder=lambda o, i: "",
    rubric=[],
    score_threshold=0.0,
...
def cheap_check_competitive_specialist(output: Any) -> CheapCheckResult:
    if not isinstance(output, dict):
        return CheapCheckResult(False, "Output must be a JSON object.")
    cl = output.get("competitive_landscape")
    if not isinstance(cl, dict):
        return CheapCheckResult(False, "'competitive_landscape' must be an object.")
    for k in ("competitors_mentioned", "win_signals", "loss_signals", "positioning_gaps"):
        if not isinstance(cl.get(k, []), list):
            return CheapCheckResult(False, f"competitive_landscape.{k} must be a list.")
    return CheapCheckResult(True)


COMPETITIVE_SPECIALIST_STEP_CONFIG = StepConfig(
    name="insights_competitive_specialist",
    cheap_check=cheap_check_competitive_specialist,
    manager_prompt="",
    manager_context_builder=lambda o, i: "",
    rubric=[],
    score_threshold=0.0,
